...
```sh
docker run -p 3001:3001 --gpus all mephodybro/polygraph_demo:0.0.5
```
The server should be available on `http://localhost:3001`

If you want to use host huggingface checkpoints, mount the volume with `-v`, e.g. like this
```sh
docker run -p 3001:3001 -v $HOME/.cache/huggingface/hub:/root/.cache/huggingface/hub --gpus all mephodybro/polygraph_demo:0.0.5
```
