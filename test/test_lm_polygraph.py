...
def run_config_with_overrides(config_name, **overrides):
    command = (
        f"HYDRA_CONFIG={pwd()}/configs/{config_name}.yaml polygraph_eval"
    )
...
def test_pe_ensembles_dont_fail():
    exec_result = run_config_with_overrides('test_polygraph_eval_ensemble',
                                            ensembling_mode='pe')
    assert (
            exec_result.returncode == 0
    ), f"polygraph_eval returned code {exec_result.returncode} != 0"
...
def test_pe_ensembles_has_all_ensemble_estimates():
    run_config_with_overrides('test_polygraph_eval_ensemble',
                              ensembling_mode='pe')
    ...
    for estimator in expected_estimators:
        ...
        assert (
            len(man.estimations[key]) > 0
        ), f"result doesn't have {key}"
...
def test_pe_ensembles_no_nans():
    ...
    for estimator in expected_estimators:
        key = ('sequence', str(estimator))
        assert not (
            np.any(np.isnan(man.estimations[key]))
        ), f"result has NaNs in {key}"
