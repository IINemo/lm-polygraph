...
def parse_args():
    ...
    parser.add_argument('--dataset', choices=DATASET_CONFIG.keys(), required=True, help='Dataset to build')
...
def main():
    ...
    if not args.save_to_disk and not args.publish:
        print("WARNING: --publish and --save-to-disk are not specified, so dataset will not be stored anywhere")
    ...
    if args.publish:
        ...
        dataset.push_to_hub(path, subset, set_default=False, token=os.environ["HF_TOKEN"])
