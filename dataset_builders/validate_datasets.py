...
def compare_columns(a_ds: datasets.Dataset, b_ds: datasets.Dataset, col: str) -> List[int]:
    diffs: List[int] = []
    a_col = a_ds[col]
    b_col = b_ds[col]
...
def main():
    p = argparse.ArgumentParser(description="Validate updated datasets against reference for input/output equality")
    p.add_argument("--ref-namespace", default="LM-Polygraph", help="Reference namespace (default: LM-Polygraph)")
    p.add_argument("--new-namespace", required=True, help="Namespace with updated datasets to validate")
    p.add_argument("--datasets", nargs="*", default=None, help="Subset of dataset keys to validate (defaults to all)")
    args = p.parse_args()
    ...
    for key in keys:
        ...
        if not common:
            print(f"[WARN] No common splits. ref={sorted(ref_splits)} new={sorted(new_splits)}")
            continue
        ...
        for split in common:
            ...
            if len(ref_split) != len(new_split):
                print(f"[ERR] {key}/{split}: row count mismatch: {len(ref_split)} != {len(new_split)}")
                any_errors = True
                continue

            for col in ("input", "output"):
                if col not in ref_split.column_names or col not in new_split.column_names:
                    print(f"[ERR] {key}/{split}: column '{col}' missing in one of the datasets")
                    any_errors = True
                    continue
                diffs = compare_columns(ref_split, new_split, col)
                if diffs:
                    print(f"[ERR] {key}/{split}: column '{col}' differs at indices: {diffs[:5]} (showing up to 5)")
                    any_errors = True
                else:
