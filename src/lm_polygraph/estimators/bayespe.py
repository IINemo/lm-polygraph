...
class BayesPEFewShot(Estimator):
    ...
    def __call__(self, stats: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        return np.array(uncertainties) import numpy as np
from typing import List, Dict, Optional
from .estimator import Estimator
from lm_polygraph.utils.common import polygraph_module_init
from scipy.optimize import minimize
...
class BayesPEZeroShot(Estimator):
    ...
    def optimize_weights(self, validation_texts: List[str], validation_labels: List[int]):
        ...
        def objective(weights):
            ...
            # Get predictions for each instruction
            predictions = []
            for instruction in self.instructions:
                ...
                # Get model predictions (placeholder - needs to be implemented with actual model)
                pred = np.random.rand(len(validation_texts), len(set(validation_labels)))  # Placeholder
                predictions.append(pred)
    ...
    def __call__(self, stats: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        for text in texts:
            ...
            for instruction in self.instructions[:self.n_forward_passes]:
                ...
                # Get model prediction (placeholder - needs to be implemented with actual model)
                pred = np.random.rand(len(set(self.instructions)))  # Placeholder
                predictions.append(pred)
            ...
            # Calculate uncertainty as entropy of ensemble prediction
            uncertainty = -np.sum(ensemble_pred * np.log(ensemble_pred + 1e-10))
            uncertainties.append(uncertainty)
...
class BayesPEFewShot(Estimator):
    ...
    def optimize_weights(self, validation_texts: List[str], validation_labels: List[int]):
        ...
        def objective(weights):
            ...
            predictions = []
            for instruction in self.instructions:
                # Format prompt with instruction and examples
                prompts = [self.prompt_formatting.format(
                    classes="\n".join([f"{i+1}. {c}" for i, c in enumerate(set(validation_labels))]),
                    examples=self._format_examples(),
                    text=text
