...
class SemanticEntropyAdaptedSampling(Estimator):
    def __init__(self, deberta_path: str = 'microsoft/deberta-large-mnli'):
        super().__init__(
            ['adapted_sample_log_probs', 'adapted_sample_log_probs_gen', 'adapted_sample_texts'],
            'sequence')
    ...
    def __call__(self, stats: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        return self.semantic_entropy.batched_call(texts, log_p, log_weights=np.array(log_p) - np.array(log_p_gen))
