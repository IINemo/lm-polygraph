...
def rank(target_array, source_array):
    ranks_array = np.array([(x >= source_array).sum() for x in target_array]) / len(source_array)
    return ranks_array
...
class PPLMDSeq(Estimator):
    def __init__(self, embeddings_type: str = "decoder", md_type: str = "MD", parameters_path: str = None, normalize: bool = False):
        ...
        if self.md_type == "MD":
            ...
            self.MD = RelativeMahalanobisDistanceSeq(embeddings_type, parameters_path, normalize=False)
            self.MD_val = RelativeMahalanobisDistanceSeq(embeddings_type, parameters_path, normalize=False)
        else:
            raise NotImplementedError
            
        if self.parameters_path is not None:
            self.full_path = f"{self.parameters_path}/ppl_{self.md_type}_{self.embeddings_type}"
