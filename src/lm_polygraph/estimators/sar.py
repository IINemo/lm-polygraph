...
class SAR(Estimator):
    ...
    def __init__(self, verbose: bool = False):
        super().__init__(
            ["sample_sentence_similarity", "sample_log_likelihoods", "sample_token_similarity"], "sequence"
        )
    ...
    def __call__(self, stats: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        SAR = []
        for batch_data in zip(batch_sample_log_likelihoods, batch_sample_token_similarity, batch_sample_sentence_similarity):
            sample_log_likelihoods = batch_data[0]
            sample_token_similarity = batch_data[1]
            sample_sentence_similarity = batch_data[2]
            
            tokenSAR = []
            for log_likelihoods, token_similarity in zip(sample_log_likelihoods, sample_token_similarity):
            ...
            R_s = probs_token_sar * sample_sentence_similarity * (1 - np.eye(sample_sentence_similarity.shape[0]))
