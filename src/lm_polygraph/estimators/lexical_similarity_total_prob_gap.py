...
class LexicalSimilarityTotalProbGap(Estimator):
    """
    ...
    def __init__(self, metric: str = "rougeL"):
        ...
        if self.metric.startswith("MIS"):
            from mutual_implication_score import MIS
            self.scorer = MIS(device='cuda')
        ...
        super().__init__(["sample_log_probs", "greedy_log_likelihoods"], "sequence")

    def __str__(self):
        return f"LexicalSimilarityTotalProbGap_{self.metric}"

    def _score_single(self, t1: str, t2: str):
        if self.metric.startswith("rouge"):
            return self.scorer.score(t1, t2)[self.metric].fmeasure
        elif self.metric.startswith("MIS"):
            return self.scorer.compute([t1], [t2])
        elif self.metric.startswith("BART"):
            return self.scorer.score([t1], [t2])
    ...
    def __call__(self, stats: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        res = []
        for probs, greedy_probs in zip(batch_probs, greedy_log_likelihoods):
            prob = np.sum(greedy_probs)
            total_probs = probs + [prob]
            res.append(np.max(total_probs) - np.min(total_probs))
        return np.array(res)
