...
class CDegMat(Estimator):
    ...
    def __init__(
        ...
        if similarity_score == "NLI_score":
            if affinity == "entail":
                super().__init__(["greedy_semantic_matrix_entail", "sample_texts"], "sequence")
            else:
                super().__init__(["greedy_semantic_matrix_contra", "sample_texts"], "sequence")
        else:
            raise ValueError("CDegMat only supports NLI_score as similarity_score.")
    ...
    def __str__(self):
        return f"CDegMat_{self.similarity_score}_{self.affinity}"

    def C_DegMat(self, i, stats):
        if self.affinity == "entail":
            W = stats["greedy_semantic_matrix_entail"][i, :]
        else:
            W = 1 - stats["greedy_semantic_matrix_contra"][i, :]
    ...
    def __call__(self, stats: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        for i, answers in enumerate(stats["sample_texts"]):
            if self.verbose:
                log.debug(f"generated answers: {answers}")
            res.append(self.C_DegMat(i, stats))
        return np.array(res)
