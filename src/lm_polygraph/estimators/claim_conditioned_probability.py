...
class ClaimConditionedProbability(Estimator):
    def __init__(self):
        super().__init__([f'greedy_tokens', f'greedy_tokens_alternatives', f'greedy_tokens_alternatives_nli'],
                         'sequence')
    ...
    def __call__(self, stats: Dict[str, np.ndarray]) -> np.ndarray:
        words = stats[f'greedy_tokens']
        alternatives = stats[f'greedy_tokens_alternatives']
        alternatives_nli = stats[f'greedy_tokens_alternatives_nli']
        prob_nli = []
        for sample_words, sample_alternatives, sample_alternatives_nli in zip(words, alternatives, alternatives_nli):
            sample_mnlis = []
            for word, word_alternatives, word_alternatives_nli in zip(sample_words, sample_alternatives,
