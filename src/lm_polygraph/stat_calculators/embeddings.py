        ...
        if not all_layers:
            ...
            if len(output.hidden_states) > 1:
                generated_tokens_hs = torch.cat(
                    [h[hidden_layer] for h in output.hidden_states[1:]], dim=1
                )
        else:
            input_tokens_hs = output.hidden_states[0].mean(axis=0)
            if len(output.hidden_states) > 1:
                generated_tokens_hs = torch.cat(
                    [h.mean(axis=0) for h in output.hidden_states[1:]], dim=1
                )
        if len(output.hidden_states) > 1:
            batch_embeddings_decoder = torch.cat(
                [input_tokens_hs, generated_tokens_hs], dim=1
            ).mean(axis=1)
        else:
        ...
        if use_averaging:
            if "decoder" in hidden_state:
                try:
                    ...
                    batch_embeddings_decoder = batch_embeddings_decoder.reshape(
                        batch_size, -1, agg_decoder_hidden_states.shape[-1]
                    )[:, 0]
                except:
                    if all_layers:
            ...
            if "encoder" in hidden_state:
                ...
                if ignore_padding:
                    if aggregation_method == "mean":
                        batch_embeddings = (encoder_embeddings).sum(1) / seq_lens
                    else:
                        batch_embeddings = aggregate(
                            encoder_embeddings, aggregation_method, axis=1
                        )
                else:
                    batch_embeddings = aggregate(
                        encoder_embeddings, aggregation_method, axis=1
