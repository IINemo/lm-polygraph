...
class StepsGreedyNLISimilarityCalculator(StatCalculator):
    ...
    def __call__(
        ...
        for sample_texts, greedy_text, greedy_solution, input_text in zip(
            ...
            batch_pairs += list(itertools.product([prefix + x for x in sample_steps], [greedy_step]))
        ...
        original_probs: list[dict] = [prob_dict[first_text, second_text] for first_text, second_text in original_order]
        ...
        for sample_texts, greedy_text in zip(batch_texts, greedy_texts):
            ...
            for k, k_id in labels_dict.items():
                steps_greedy_nli_similarity[-1][f'forward_{k}'] = [f[k_id].item() for f in fwd_sample_probs]
                steps_greedy_nli_similarity[-1][f'backward_{k}'] = [f[k_id].item() for f in bwd_sample_probs]
        return {
            "steps_greedy_nli_similarity": reconstruct(steps_greedy_nli_similarity, sample_steps_texts)
        }
