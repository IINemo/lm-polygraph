...
class StepsCrossEncoderSimilarityCalculator(StatCalculator):
    ...
    def _build_cumulative_text(
        self, input_text: str, steps: List[str], current_step_idx: int
    ) -> str:
        """
    ...
    def __call__(
        ...
        for sample_idx, (
            ...
            zip(
                batch_input_texts,
                batch_sample_steps_texts,
                batch_sample_steps_tokens,
                batch_claims,
                batch_greedy_tokens,
            )
        ):
            ...
            for step_idx in range(n_steps):
                ...
                step_texts = sample_steps_texts[
                    step_idx
                ]  # All sample texts for this step
                ...
                step_tokens = sample_steps_tokens[
                    step_idx
                ]  # All sample tokens for this step
                    ...
                    if len(sample_tokens) > 1:
                        ...
                        cropped_tokens = list(
                            itertools.combinations(
                                sample_tokens, len(sample_tokens) - 1
                            )
                        )[::-1]
                        ...
                        raw_text = (
                            base_cumulative
                            + " "
                            + tokenizer.decode(sample_tokens, skip_special_tokens=True)
                        )
                ...
                if step_idx < len(claims):
                    ...
                    step_start = (
                        step_idx * (len(greedy_tokens) // n_steps) if n_steps > 0 else 0
                    )
                    step_end = min(
                        (step_idx + 1) * (len(greedy_tokens) // n_steps),
                        len(greedy_tokens),
                    )
                    ...
                    if len(step_greedy_tokens) > 1:
                        ...
                        cropped_tokens = list(
                            itertools.combinations(
                                step_greedy_tokens, len(step_greedy_tokens) - 1
                            )
                        )[::-1]
                        ...
                        raw_text = (
                            ...
                            + " "
                            + tokenizer.decode(
                                step_greedy_tokens, skip_special_tokens=True
                            )
                        )
                        ...
                        token_scores = (
                            np.array([0.5] * len(step_greedy_tokens))
                            if len(step_greedy_tokens) > 0
                            else np.array([0.5])
                        )
            ...
            batch_steps_sample_sentence_similarity.append(
                steps_sample_sentence_similarity
            )
