                ...
                if token == pad_token_id:
                    ...
                    if aggregate_models:
                        for i, model_logits in enumerate(
                            ...
                            model_posterior = (
                                model_logits[obs_id, seq_i].cpu().softmax(-1)
                            )
                    ...
                    entropies["entropy_top5"] = entropy(
                        posterior.topk(5, dim=-1).values
                    )
                    entropies["entropy_top10"] = entropy(
                        posterior.topk(10, dim=-1).values
                    )
                    entropies["entropy_top15"] = entropy(
