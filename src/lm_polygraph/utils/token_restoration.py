...
def collect_sample_token_level_uncertainties(
    ...
    token_level_uncertainties = {
        key: torch.zeros(unc_shape) for key in token_measures
    }
    ...
    if aggregate_models:
        ...
        models_sequence_scores = torch.zeros(batch_size, num_models, num_return_sequences, seq_length)
...
def collect_token_level_uncertainties(
    ...
    if aggregate_models:
        modelwise_penalties = seq_penalty.unsqueeze(1).repeat(1, models_sequence_scores.shape[1], 1)
