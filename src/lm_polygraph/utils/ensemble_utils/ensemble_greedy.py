        ...
        **model_kwargs,
        ...
        while True:
            ...
            if self.models_hypo_logits_iter is None:
                self.models_hypo_logits_iter = torch.zeros((num_models, batch_size, 1)).to(input_ids.device)
                models_hypo_logits = self.models_hypo_logits_iter

            denom = models_hypo_logits.logsumexp(dim=0)
            num = (torch.stack(models_next_token_logits) + models_hypo_logits).logsumexp(dim=0)
            ...
            token_models_hypo_logits = torch.gather(token_models_hypo_logits, -1,
                                                    next_tokens.repeat((num_models), 1).unsqueeze(-1))

            self.models_hypo_logits_iter = torch.cat((self.models_hypo_logits_iter,
                                                      token_models_hypo_logits), -1)
