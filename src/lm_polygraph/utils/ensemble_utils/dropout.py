...
def functional_dropout_share(input: Tensor, p: float = 0.5, training: bool = True, inplace: bool = False) -> Tensor:
    if not training:
        return input
    shape = input.shape
    mask_shape = shape[2:]
...
def functional_dropout(input: Tensor, p: float = 0.5, training: bool = True, inplace: bool = False) -> Tensor:
    if not training:
        return input
    shape = input.shape
    mask_shape = shape[1:]
